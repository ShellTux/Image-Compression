from common import TEST_DOWNSAMPLING, TEST_IMAGE
...
def test_downsample_ycbcr():
    ...
    _, intermidiate_values = encoder.encoder(image, downsampling=TEST_DOWNSAMPLING, return_intermidiate_values=True)

    image_ycbcr = intermidiate_values['ycbcr']

    # Aplicar downsampling
    Y_d, Cb_d, Cr_d = downsample_ycbcr(image_ycbcr, sampling=TEST_DOWNSAMPLING, interpolation=cv2.INTER_CUBIC)
