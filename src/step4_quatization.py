from common import DOCS_DIR, IMAGES, DEFAULT_DOWNSAMPLE, generate_path
from itertools import product
from matplotlib import pyplot as plt
import encoder
import numpy as np

def get_quantization_matrix(quality_factor: int = 50) -> np.ndarray:
    """
    ...
    quantization_matrix = (jpeg_quantization_matrix * scale_factor).round().clip(1, 255).astype(np.uint8)
    return quantization_matrix

def quantization(dct_image: np.ndarray, quality_factor: int = 100, block_size: int = 8) -> np.ndarray:
    """
    ...
    return quantized_image.astype(np.int32)

def iquantization(quantized_image: np.ndarray, quality_factor: int = 100, block_size: int = 8) -> np.ndarray:
    """
...
def main():
    for image_path in IMAGES:
        ...
        for quality_factor in (100,):
            ...
            axis[0].imshow(Y_q, cmap='gray', vmin=0, vmax=255)
            axis[0].set_title('Quantized Y Channel')
            axis[0].axis('off')

            axis[1].imshow(Cb_q, cmap='gray', vmin=0, vmax=255)
            axis[1].set_title('Quantized Cb Channel')
            axis[1].axis('off')

            axis[2].imshow(Cr_q, cmap='gray', vmin=0, vmax=255)
